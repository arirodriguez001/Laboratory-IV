...
# for freq in np.linspace(500, 50000, 50):  # Barrido frecuencias de 1 Hz a 10 kHz
...
# nombre_archivo = "medicion_lockin-nucleo-con-metal.csv"
...
for volt in np.linspace(0.05, 0.5, 20):  # Barrido frecuencias de 1 Hz a 10 kHz
    fungen.write(f"VOLTAGE:AMPLITUDE {volt}")
    ...
    print(f"Voltaje ajustado a: {volt} V")
    

datos = np.column_stack((volts, x_valores, y_valores, r_valores, theta_valores))


nombre_archivo = "medicion_volt322025.txt"
np.savetxt(nombre_archivo, datos, delimiter=',', header='Volts,X,Y,R,Theta', comments='')
